...
import com.example.geometry.databinding.ActivityParalelBinding

class Paralel : AppCompatActivity() {
        ...
        spinner.onItemSelectedListener = object:
            ...
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                ...
                if(lista[p2] == lista[1]){//Condicional para que solo pase a Activities dependiendo de lo que se escoja en arreglo.
                    val intentArea1 = Intent(this@Paralel,MainActivity::class.java)
                    startActivity(intentArea1)
                    finish()

                } else if(lista[p2] == lista[2]) {
                    val intentAltura1 = Intent(this@Paralel,Cilindro::class.java)
                    startActivity(intentAltura1)
                    finish()

                } else if(lista[p2] == lista[3]) {
                    val intentAncho = Intent(this@Paralel,Paralel::class.java)
    ...
    fun click(view: View) {
        if(!binding.etNumero.text.toString().isEmpty() && !binding.etNumero2.text.toString().isEmpty()){
            val altura = binding.etNumero.text.toString().toInt()
            val area = binding.etNumero2.text.toString().toInt()

            val volumen = (area*altura)*0.3333
            val intentResArea = Intent(this, ResultadosParalel::class.java)

            //Se pasa valor de area a activity ResultadosTriangulo
            val parametros1 = Bundle()
            parametros1.putDouble("volumen",volumen)
